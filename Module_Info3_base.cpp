...
void Module_Info::RegisterPin(tinyxml2::XMLElement* pin, module_info_pins_t* pinlist, int32_t plugin_sub_type, int& pin_id) // XML data passed in.
{
	...
	// audio for DSP pins, float for GUI/Controller pins (which have no audio datatype).
	// pin_description2 alone defaults to DT_FSAMPLE, which is only right for DSP.
