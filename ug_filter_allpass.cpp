...
void ug_filter_allpass::freq_change( float p_pitch, float p_resonance )
{
	...
	if( p_pitch < 0 )	// do it the hard way
	{
		...
		factor_A = 2.f * cos( 2.0 * M_PI * f ) * exp( - M_PI * f *q_const ) ;
		factor_B = exp( - (2.0 * M_PI) * f * q_const );
	}
	else
	{
		...
		float factor_A_exact = 2.f * cos( (2.0 * M_PI) * f ) * exp( - M_PI * f * q_const );
		assert( fabs(( factor_A_exact - factor_A ) / factor_A_exact ) < 0.01f );
		float factor_B_exact = exp( 2.f * (- M_PI * f * q_const) );
...
int ug_filter_allpass::Open()
{
	...
	if( !lookup_tableA->GetInitialised() )
	{
		...
		{
			float temp = expf( - (2.0 * M_PI) * j / (float)TABLE_SIZE );
			lookup_tableA->SetValue( j, temp );
			temp = 2.f * cosf( (2.0 * M_PI) * j / (float)TABLE_SIZE );
	...
	ug_base::Open();
	SET_PROCESS_FUNC( &ug_filter_allpass::process_all );
	delay2 = delay1 = 0.0f;
	return 0;
}
...
void ug_filter_allpass::onSetPin(timestamp_t p_clock, UPlug* p_to_plug, state_type p_state)
{
	...
	{
		SET_PROCESS_FUNC( &ug_filter_allpass::process_fixed_freq );
	}
	...
	if( output_quiet == false )
	{
		if( GetPlug(PN_SIGNAL)->getState() != ST_RUN ) // input stopped
		{
			...
			if( temp3 == delay1 || energy < INSIGNIFICANT_SAMPLE ) // y1n = current output
			{
				...
				output_quiet = true;
				ResetStaticOutput();
				SET_PROCESS_FUNC( &ug_filter_allpass::process_static );
				return;
			}
		}
