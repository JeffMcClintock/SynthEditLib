...
// should probly use a host control rather than direct access
...
public:
	...
	int32_t OnRender(GmpiDrawing_API::IMpDeviceContext* drawingContext ) override
	{
		...
		CSynthEditAppBase* application = nullptr;
		auto modview = dynamic_cast<SE2::ModuleView*>(getHost());
		assert(modview != nullptr);
		{
			...
			application = dynamic_cast<CSynthEditAppBase*>(container->Document()->Application());
		}
		...
		if(cpuPercent < 10.0f)
			snprintf(text, std::size(text), "%0.1f", cpuPercent);
		else
			snprintf(text, std::size(text), "%0.0f", cpuPercent);
...
<?xml version="1.0" ?>
<PluginList>
  <Plugin id="SE CPU Meter" name="CPU Meter" category="Debug" >
