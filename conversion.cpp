...
int StringToInt(const wstring& string, int p_base)
{
	...
	wchar_t* temp;
	return static_cast<int32_t>(wcstol( string.c_str(), &temp, p_base ));
}
...
int StringToInt(const char* string, int p_base)
{
 //   assert( string.size() < 11 || string.find(L',') != string::npos  ); // else too many digits overflows sizeof int
    char* temp;
    return static_cast<int32_t>(strtol( string, &temp, p_base ));
}
...
bool AreCompatible( EPlugDataType d1, EPlugDataType d2 )
{
	...
	// Bool and Volts are interchangeable
	if ((d1 == DT_FSAMPLE || d1 == DT_BOOL) && (d2 == DT_FSAMPLE || d2 == DT_BOOL))
		return true;

	// Float and Volts are interchangeable
	if ((d1 == DT_FSAMPLE || d1 == DT_FLOAT) && (d2 == DT_FSAMPLE || d2 == DT_FLOAT))
		return true;
	...
	*/
	...
	// etc
	if ((d1 == DT_INT || d1 == DT_FLOAT || d1 == DT_BOOL || d1 == DT_TEXT || d1 == DT_STRING_UTF8) && (d2 == DT_INT || d2 == DT_FLOAT || d2 == DT_BOOL || d2 == DT_TEXT || d2 == DT_STRING_UTF8))
		return true;

// nuh	if (d1 == DT_FSAMPLE && d2 == DT_ENUM)
//		return true;
...
std::wstring enum_get_substring (const std::wstring& enum_list, int p_value )
{
	...
	if( Left(enum_list,5) == std::wstring(L"range") )
	{
