...
//	float mid_y = floorf(0.5f + height * 0.5f);
	...
	int newSpectrumCount = -1 + static_cast<int>(pinSpectrum.rawSize() / sizeof(float));
		...
		if (height > 30)
		{
			...
			while (true)
			{
				...
				if (y > lastTextY + fontHeight * 1.2)
				{
					lastTextY = y;
					char txt[10];
					snprintf(txt, std::size(txt), "%3.0f", (float)db);
...
//				x = snapToPixelOffset + floorf(0.5f + x);
				...
				if (samplerate > 0)
				{
					...
					if (hz > 999.0)
					{
						snprintf(txt, std::size(txt), "%.0fk", hz * 0.001);
					}
					else
					{
						snprintf(txt, std::size(txt), "%.0f", hz);
					}
