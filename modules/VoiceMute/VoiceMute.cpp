...
// obsolete, use /SE_DSP/modules_internal/Voicemute !!!
...
{
	// subjectivly 10ms or more is best, faster ramps 'pop' to various degrees.
	...
	// initialise sine-shaped gain curve.
	if( needInitialize != 0 )
	{
...
void VoiceMute::onSetPins(void)  // one or more pins_ updated.  Check pin update flags to determin which ones.
{
	if( pinVoiceActive.isUpdated() )
	{
		#if defined(VM_DEBUG_OUTPUT)
			_RPT2(_CRT_WARN, "VoiceMute V%d Active=%f\n", physicalVoiceNumber, pinVoiceActive.getValue() );
		#endif
	...
	{
		// under normal procesing, output reflects input state.
...
void VoiceMute::subProcessSilence(int bufferOffset, int sampleFrames)
{
	...
	// process fidly non-sse-aligned prequell.
