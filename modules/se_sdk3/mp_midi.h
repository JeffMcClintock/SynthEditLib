...
//				static_cast<uint32_t>((msg[4] << 24) | (msg[5] << 16) | (msg[6] << 8) | msg[7])
		...
		public:
			...
			{
				// MIDI 2.0 messages need no conversion — pass through and return. Without the
				...
				case midi_1_0::status_type::ControlChange:
				{
					...
					case 6:	// Data Entry MSB — completes whichever of RPN or NRPN is active.
					{
						...
						else if (incoming_nrpn[header.channel] != NULL_RPN)
						{
							const auto msgout = gmpi::midi_2_0::makeNrpnRaw(
								incoming_nrpn[header.channel],  // was incoming_rpn — a bug: always NULL_RPN here.
						...
						{
							// No RPN/NRPN selected — data-entry CC 6 is a plain CC in its own right.
					...
					case 38:	// Data Entry LSB — completes the current RPN/NRPN with fine resolution.
					{
						...
						{
							// See the CC 6 MSB branch — CC 38 on its own is a plain CC too.
							const auto msgout = gmpi::midi_2_0::makeController(
								controller.controllerNumber,
								controller.value,
								header.channel
