...
#pragma warning( push )
#pragma warning( disable : 6011 ) // Dereferencing NULL pointer 'e' (false flag)
...
#pragma warning( pop ) // Dereferencing NULL pointer 'e' (false flag)
...
void MidiOutPin::send(const unsigned char* data, size_t size, int blockPosition)
{
	assert( blockPosition >= -1 && "MIDI Out pin can't use negative timestamps" );
	if( blockPosition == -1 )
	{
	...
	plugin_->getHost()->setPin(blockPosition, getId(), static_cast<int32_t>(size), data);
}
