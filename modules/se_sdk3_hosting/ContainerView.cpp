...
//		_RPT0(_CRT_WARN, "~ContainerView()\n");
	...
	void ContainerView::setDocument(SE2::IPresenter* ppresentor, int pviewType)
	{
		viewType = pviewType;
	...
	void ContainerView::Refresh(Json::Value* context, std::map<int, SE2::ModuleView*>& guiObjectMap)
	{
...
//				const int lastYgrid = gridSize * gridBoarder + largeGridSize * (FastRealToIntTruncateTowardZero(drawingBounds.getHeight() / largeGridSize - 1));
	...
	int32_t ContainerView::populateContextMenu(float x, float y, gmpi::IMpUnknown* contextMenuItemsSink)
	{
	...
	int32_t ContainerView::onContextMenu(int32_t idx)
	{
		return GmpiSdk::ContextMenuHelper::onContextMenu(contextMenuCallbacks, idx);
	}

	int32_t ContainerView::onPointerUp(int32_t flags, GmpiDrawing_API::MP1_POINT point)
	{
...
*/
	...
	void ContainerView::DragNewModule(const char* id)
	{
