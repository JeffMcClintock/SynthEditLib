...
//
// Shared machinery for the Butterworth filter modules built on the iir1
// library - the maintained successor of the 'DspFilters' library that the
// deprecated 'Butterworth ... 2' modules use. See iir1/README-SynthEdit.md
// for provenance and local fixes.
//
// These are classic fixed-design IIR filters: the cutoff is a control-rate
// value in Hz (not a 1V/Oct audio signal), and recalculating a high-order
// design is too expensive (and too glitchy) to do per-sample. Instead, when a
// parameter changes the module sets up the new design on a spare filter
// instance and fades across to it: first the input is faded up into the new
// filter (so its start-up transient dies away quietly), then the output is
// crossfaded from the old design to the new one - 50 ms per phase.
// Ported from the previous generation (modules/Filters/IIR_Filters2.cpp).
...
struct ButterworthBase : public gmpi::FilterBase
{
	// Every variant starts with these two pins. The remaining pins
	// (Width/Gain/Poles/Output) are declared by the concrete module: GMPI pins
	// register in member-construction order, so the derived class's pins land
	// after these, keeping each module's pin order identical to its XML.
	...
	void initButterworth(gmpi::EnumInPin& poles, gmpi::AudioOutPin& output)
	{
		polesPin = &poles;
		outputPin = &output;
	...
	void subProcess(int sampleFrames)
	{
		...
		if constexpr (isCrossfading)
		{
			if (crossfade >= 1.0f)
			{
				...
				setSubProcess(&ButterworthBase::subProcess<false>); // done crossfading
		...
		else
		{
			if (filterUpdated)
			{
				...
				setSubProcess(&ButterworthBase::subProcess<true>); // start crossfading
	...
	void onGraphStart() override
	{
		...
		setSubProcess(&ButterworthBase::subProcess<false>);
		outputPin->setStreaming(true);
	}

	// Concrete modules with Width/Gain pins override this to extend the change check.
	virtual bool designParamsUpdated()
	{
		return pinPitch.isUpdated() || polesPin->isUpdated();
	}
