...
namespace
{
REGISTER_MODULE_1(L"Slider", IDS_MN_SLIDER, IDS_MG_OLD,ug_slider ,CF_STRUCTURE_VIEW|CF_PANEL_VIEW,L"Allows direct control of an input voltage.  For example connect one to an Oscillators's 'Pitch' input.  Can appear as a knob or button as well (See 'Appearance' parameter). Can be configured to send any MIDI controller or NRPN number, handy for controlling external MIDI gear.");
}
...
void ug_slider::onSetPin(timestamp_t p_clock, UPlug* p_to_plug, state_type )
{
	if (p_to_plug == GetPlug(PLG_PATCH_VALUE))
	{
		...
		SET_PROCESS_FUNC(&ug_slider::sub_process);
	}
}
