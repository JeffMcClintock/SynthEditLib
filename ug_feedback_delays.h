...
{
public:
	DECLARE_UG_BUILD_FUNC(ug_lookahead2);
	DECLARE_UG_INFO_FUNC2;

	ug_lookahead2();
	int Open() override;
	int calcDelayCompensation() override; // latency known from the 'ms' pin default, pre-processing
	void onSetPin(timestamp_t p_clock, UPlug* p_to_plug, state_type p_state) override;
	void sub_process(int start_pos, int sampleframes);
	void sub_process_static(int start_pos, int sampleframes);
...
public:
	...
	void ListInterface2(std::vector<class InterfaceObject*>&PList) override
	{
		if constexpr (DIRECTION == DR_IN)
		{
			...
			PList.push_back(new_InterfaceObjectA(nullptr, (L"In"), DR_IN, (EPlugDataType)DATATYPE, L"", L"", 0, L""));
		}
		else
		{
		...
		PList.push_back( new_InterfaceObjectA( nullptr, (L"Out"), DR_OUT, (EPlugDataType)DATATYPE, L"", L"",0, L"") );
