...
#endif
{
	...
	if( ( pdesc.GetFlags() & IO_PATCH_STORE ) != 0)
	{
...
void UPlug::ReRoute()
{
	...
	while( !connections.empty() )
	{
		...
		for( auto it = tiedto->connections.begin(); it != tiedto->connections.end(); ++it )
		{
			UPlug* otherPlug = *it;
...
void UPlug::SetBufferValue( const char* p_val )
{
	...
	case DT_INT:
	{
		...
		*((int*) io_variable) = static_cast<int32_t>(strtol( p_val, &temp, 10 ));
	}
	break;
