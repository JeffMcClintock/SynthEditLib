...
int32_t ug_plugin3Base::setPin(int32_t blockRelativeTimestamp, int32_t pinIndex, int32_t size, const void* data)
{
	...
	if (pin->Direction != DR_OUT)
	{
		...
		oss << L"Error: " << moduleType->m_name << L" sending data out INPUT pin.";
	...
	{
		std::wostringstream oss;
		oss << L"Error: " << moduleType->m_name << L" sending data with negative timestamp. Please inform module vendor.";
		AudioMaster2()->ugmessage( oss.str().c_str());
		return gmpi::MP_FAIL;
	}
...
int32_t ug_plugin3Base::setPinStreaming( int32_t blockRelativeTimestamp, int32_t pinIndex, int32_t is_streaming)
{
	...
	if(pin->Direction != DR_OUT)
	{
		AudioMaster2()->ugmessage(moduleType->m_name + L" Error: DSP Module setting streaming on INPUT pin");
		return gmpi::MP_FAIL;
	}
...
int32_t ug_plugin3Base::createPinIterator(gmpi::IMpPinIterator** returnIterator )
{
	if( !GetFlag(UGF_OPEN) )
	{
		AudioMaster2()->ugmessage( moduleType->m_name + L": Can't iterate pins in constructor. use open() method instead.");
	}
...
{
	_RPTW1(_CRT_WARN, L"%s ", moduleType->m_name.c_str() );
}
#endif
