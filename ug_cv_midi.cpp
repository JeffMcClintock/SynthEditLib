...
namespace
{
REGISTER_MODULE_1(L"Trigger to MIDI", IDS_MN_TRIGGER_TO_MIDI,IDS_MG_OLD,ug_cv_midi ,CF_STRUCTURE_VIEW,L"Sends a MIDI note-on message whenever the 'Trigger' input goes over 0 Volts.");
}
...
//		unsigned int midi_data;
		uint8_t chan = (uint8_t) max( (short)0, channel ); // chan -1 -> chan 0
