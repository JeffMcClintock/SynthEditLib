...
namespace VstPresetUtil
{
	...
	void WritePreset(std::wstring wfilename, std::string categoryName, std::string vendorName, std::string productName, const char* processorId, std::string xmlPreset)
	{
...
//		std::string filename_asci = WStringToMultibyte(wfilename);
		...
		if (stream)
		{
			...
			assert(memcmp(classID.toTUID(), processorGuid, sizeof(TUID)) == 0);
#endif

			Steinberg::Vst::PresetFile::savePreset(stream, classID, &proxy, 0, vst3MetaInfoPrinter.CStr(), (Steinberg::int32) vst3MetaInfoPrinter.CStrSize() - 1); // CStrSize(): Note the size returned includes the terminating null.
			stream->release();
		}
	}
