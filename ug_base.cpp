...
UPlug* ug_base::GetPlugById(int id)
{
	...
	if (auto& pindesc = getModuleType()->plugs; !pindesc.empty())
	{
		...
		{
			const int pinIndex = pindesc.size() + id - last->getPlugDescID() - 1;
...
//		clone->parent_container = parent;
		...
		UGLookupList.insert({ this, clone });
	}
...
#endif				
				if(!p->isUiPlug() && /*!p->isCusto misable(0) &&*/ !p->autoDuplicate())
				{
...
#endif				
				if( !p->isUiPlug() && /*!p->isCust omisable(0) &&*/ !p->autoDuplicate())
				{
					auto up = new UPlug( this, *p );
					assignPlugVariable( p->getPlugDescID(), up );
					AddPlug( up );
				}
			}
		}
		...
		for( TiXmlElement* plugElement = plugsElement->FirstChildElement(); plugElement; plugElement=plugElement->NextSiblingElement())
		{
			...
			else
			{
				...
				if (plugElement->QueryIntAttribute("AutoCopy", &autoCopyDescId) == TIXML_SUCCESS)
				{
					...
					if (pinDescription)
					{
						assert(pinDescription->autoDuplicate());
/*
* ??? STUPID ???
...
void ug_base::HookUpParameters()
{
	...
	for (auto pmap : getModuleType()->plugs)
	{
		...
		if (!pinfo->isUiPlug()) // DSP pins.
		{
			// Special case for host-controlled legacy module pins ( ug_patch_param_watcher, voice-mute only )
			if (pinfo->isHostControlledPlug())
			{
				assert(pinfo->getHostConnect() != HC_NONE); // older method relyed on StringToHostControl(p->getName()); should be fixed already in module_info.

				auto pin = GetPlug(dspIdx);
				if (pin->Direction == DR_OUT)
				{
					// Still using old dodgy method. TODO: Update as below.
					ParentContainer()->GetParameterWatcher()->CreatePlug(pinfo->getHostConnect(), pin);
				}
				else
				{
					...
					ParentContainer()->ConnectHostControl(pinfo->getHostConnect(), pin);
				}
			}

			// handle parameter plugs.
			if (pinfo->isParameterPlug())
			{
				/* should now be handled by module_info
				if (pinfo->getParameterId() < 0)
					pinfo->setParameterId(0); // assuming SDK1 modules use only one parameter max.
				*/
				assert(pinfo->getParameterId() >= 0);
				...
				if (pin->Direction == DR_OUT)
				{
					ParentContainer()->GetParameterWatcher()->CreateParameterPlug(Handle(), pinfo->getParameterId(), pin);
				}
				else
				{
					ParentContainer()->GetParameterSetter()->ConnectParameter(Handle(), pinfo->getParameterId(), pin);
				}
			}
			++dspIdx;
		}
...
void ug_base::CopyConnectorsFrom( ug_base* FromUG, CUGLookupList& UGLookupList )
{
	...
	for( auto it = FromUG->plugs.begin() ; it != FromUG->plugs.end() ; ++it )
	{
		UPlug* p = *it;
	...
	for( auto p : plugs )
	{
		if( p->Direction == DR_IN && !p->InUse() && p->DataType != DT_MIDI && p->Proxy == 0 ) // MIDI-CV redirects MIDI-Chan. Don't set it (crash).
		{
			...
			if( moduleType ) // oversamplers have null moduletype. But don't have default values anyhow.
			{
				...
				if (desc)
				{
					defaultVal = WStringToUtf8(desc->GetDefaultVal());
				}
			}
...
{
	bool canSleep = true;
	for( auto it = outPlug->connections.begin(); it != outPlug->connections.end(); ++it )
	{
		UPlug* to = *it;
