...
#include "modules/se_sdk3_hosting/ProcessorStateManager.h"
		...
		case CONTROL_CHANGE:
		{
			...
			if (useTwoByteMidiCcs && midi_controller_id < 64) // handle 14 bit (two byte) CCs.
			{
				...
				if (base_midi_controller_id == RPN_CONTROLLER) // data entry slider (RPN/NRPN)
				{
					...
					auto it = m_rpn_memory.find(rpn_key);
...
void DspPatchManager::vst_setAutomationId(dsp_patch_parameter_base* p_param, int p_controller_id)
{
	...
	for(auto it = vst_automation_map.begin(); it != vst_automation_map.end(); ++it )
	{
...
//			lookupController = lookupController & 0xff000000; // mask off key number
		...
		vst_automation_map.insert({ lookupController, p_param });
	}
	else
	{
...
void DspPatchManager::getPresetState( std::string& chunk, bool saveRestartState)
{
	...
	{
		char buffer[20];
		snprintf(buffer, std::size(buffer), "%08x", BundleInfo::instance()->getPluginId());
...
void DspPatchManager::setPresetState(const std::string& chunk, bool isAsyncRestart)
{
	...
	if ( doc.Error() )
	{
		...
		//Application()->SeMessageBox( oss.str().c_str(), L"", MB_OK|MB_ICONSTOP );
